...
# Admin role check
# Keep this early so non-admin users cannot access management data/actions.
if not is_admin():
    ...
    try:
        ...
        if not docs_df.empty:
            ...
            if selected_doc_id:
                ...
                # Two-step confirmation reduces accidental destructive deletes.
...
with tab3:
    ...
    with col1:
        ...
        if st.button("Save Gap Detection Settings", use_container_width=True):
            # Store as config values so detection thresholds are consistent across sessions.
